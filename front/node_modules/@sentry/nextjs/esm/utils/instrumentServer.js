...
import { configureScope, captureException, deepReadDirSync, getCurrentHub, addRequestDataToEvent, startTransaction } from '@sentry/node';
import { hasTracingEnabled, extractTraceparentData, getActiveTransaction } from '@sentry/tracing';
import { fill, addExceptionMechanism, isString, logger, parseBaggageSetMutability, stripUrlQueryAndFragment } from '@sentry/utils';
...
function getPublicDirFiles() {
  try {
    ...
    // - be URL-encoded
    var dirContents = deepReadDirSync(liveServer.publicDir).map(filepath =>
      ...
      if (currentScope) {
        ...
        if (hasTracingEnabled() && shouldTraceRequest(nextReq.url, publicDirFiles)) {
          ...
          if (nextReq.headers && isString(nextReq.headers['sentry-trace'])) {
            ...
            (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && logger.log(`[Tracing] Continuing trace ${_optionalChain([traceparentData, 'optionalAccess', _2 => _2.traceId])}.`);
          }

          var rawBaggageString = nextReq.headers && isString(nextReq.headers.baggage) && nextReq.headers.baggage;
          var baggage = parseBaggageSetMutability(rawBaggageString, traceparentData);
